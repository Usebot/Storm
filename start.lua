sudo_Storm = dofile("sudo.lua")
https = require("ssl.https")
JSON = dofile("./libs/JSON.lua")
local Storm_dev = io.open("Storm_online.lua")
if Storm_dev then
Storm_on = {string.match(Storm_dev:read('*all'), "^(.*)/(%d+)")}
local tsheke_file = io.open("sudo.lua", 'w')
tsheke_file:write("token = '" ..Storm_on[1].."'\n\nsudo_add = "..Storm_on[2].."" )
tsheke_file:close()
https.request("https://api.telegram.org/bot"..Storm_on[1].."/sendMessage?chat_id="..Storm_on[2].."&text=Bot_Storm_is_start_new")
os.execute('cd .. && rm -rf .telegram-cli')
os.execute('rm -rf Storm_online.lua')  
os.execute('./tg -s ./Storm.lua $@ --bot='..Storm_on[1])
end
...
os.execute('cd && rm -fr .telegram-cli')
os.execute('./tg -s ./Storm.lua $@ --bot='..tokenCk)
else
...
local data = JSON:decode(req)
if data.ok == true then
os.execute('./tg -s ./Storm.lua $@ --bot='..token)
else
